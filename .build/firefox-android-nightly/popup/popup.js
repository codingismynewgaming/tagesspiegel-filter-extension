// Tagesspiegel Customizer - Popup Script
...
const headlinesFontSizeSelect = document.getElementById('headlinesFontSize');
const fontEnabledToggle = document.getElementById('fontEnabledToggle');
...
async function loadSettings() {
  try {
    ...
    bodyFontSizeSelect.value = String(result.fontSizeBody);
    headlinesFontSizeSelect.value = String(result.fontSizeHeadlines);
    fontEnabledToggle.checked = result.fontEnabled !== false;
    ...
    hiddenSections = [];
    hiddenSectionIds = [];
    fontEnabledToggle.checked = true;
  }
}
...
function updateFontControlsState() {
  const isEnabled = fontEnabledToggle.checked;
  bodyFontSizeSelect.disabled = !isEnabled;
  headlinesFontSizeSelect.disabled = !isEnabled;
...
function setupEventListeners() {
  ...
  fontEnabledToggle.addEventListener('change', async (event) => {
    await saveSetting('fontEnabled', event.target.checked);
    updateFontControlsState();
    showStatusMessage('Settings saved!');
  });

  bodyFontSizeSelect.addEventListener('change', async (event) => {
    await saveSetting('fontSizeBody', parseInt(event.target.value, 10));
    showStatusMessage('Settings saved!');
  });

  headlinesFontSizeSelect.addEventListener('change', async (event) => {
    await saveSetting('fontSizeHeadlines', parseInt(event.target.value, 10));
    showStatusMessage('Settings saved!');
  });
