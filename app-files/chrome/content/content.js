...
(function() {
  if (typeof browser === 'undefined') {
    ...
    script.src = chrome.runtime.getURL('browser-polyfill.js');
    script.onload = function() {
      // Polyfill loaded, now initialize the main content script
      initContentScript();
    };
    (document.head || document.documentElement).appendChild(script);
  } else {
    // Browser already supports 'browser' API (Firefox)
    initContentScript();
  }
  ...
  const STAGE_BLOCK_SELECTOR = 'section[data-bi-stage-type], aside[data-bi-stage-type]';
  ...
  function isLikelySectionLandingHeading(text) {
    if (!text) {
      return false;
    }
  ...
  function getSectionLandingId(detected) {
    if (isFirefoxBrowser()) {
      return '';
    }
    ...
    const stageBlocks = main.querySelectorAll(STAGE_BLOCK_SELECTOR).length;
    if (stageBlocks < 3) {
      return '';
    }
    ...
    if (!isLikelySectionLandingHeading(headingText)) {
      return '';
    }

    return canonicalizeSectionId(headingText);
  }
  ...
  function hideElementDirect(element, sectionId, isInitialHide = false) {
  ...
  function hideSectionLandingBlocks(sectionId, isInitialHide = false) {
    const main = document.querySelector('main#main-content');
    if (!main) {
      return 0;
    }
    ...
    let hiddenCount = 0;
    const blocks = main.querySelectorAll(STAGE_BLOCK_SELECTOR);
    blocks.forEach((block) => {
      if (hideElementDirect(block, sectionId, isInitialHide)) {
        hiddenCount += 1;
      }
    });

    if (hiddenCount > 0) {
      console.log(`[Tagesspiegel Filter] Section landing hide for "${sectionId}": ${hiddenCount}`);
    }
  ...
  function hideSections(hiddenIds, isInitialHide = false) {
    ...
    const normalizedHiddenIds = new Set(hiddenIds.map((id) => canonicalizeSectionId(id)).filter(Boolean));
    const sectionLandingId = getSectionLandingId(detectedSections);
    if (sectionLandingId && normalizedHiddenIds.has(sectionLandingId)) {
      hideSectionLandingBlocks(sectionLandingId, isInitialHide);
    }
