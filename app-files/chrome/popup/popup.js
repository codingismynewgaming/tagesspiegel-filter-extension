...
function setupEventListeners() {
  ...
  const autoSortCommentsToggle = document.getElementById('autoSortComments');
  if (autoSortCommentsToggle) {
    autoSortCommentsToggle.addEventListener('change', async (event) => {
      autoSortComments = event.target.checked;
      await saveSettings();
      showStatusMessage(autoSortComments ? 'Auto-Sort aktiviert!' : 'Auto-Sort deaktiviert!');
    });
  }
}
