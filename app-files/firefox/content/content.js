...
(function() {
  ...
  // ========== Coral Comment Sort Feature ==========
  
  const CORAL_CONFIG = {
    toastDuration: 5000, // 5 seconds
    hasShownToast: false // Track per page load
  };

  async function showCommentSortToast() {
    // Don't show multiple times on same page
    if (CORAL_CONFIG.hasShownToast) {
      return;
    }

    // Check if feature is enabled
    const { autoSortComments } = await browser.storage.sync.get({
      autoSortComments: true
    });
    
    if (autoSortComments === false) {
      return;
    }
    
    // Check if Coral comments are present and visible
    if (!detectCoralComments()) {
      return;
    }

    // Mark as shown
    CORAL_CONFIG.hasShownToast = true;
    
    // Create toast element
    const toast = document.createElement('div');
    toast.className = 'ts-customizer-toast';
    toast.innerHTML = `
      <span class="ts-customizer-toast-icon">💡</span>
      <span class="ts-customizer-toast-message">
        Auto-Sortierung fehlgeschlagen. Tipp: Klicken Sie auf 'Sortieren' → 'Am meisten respektiert'
      </span>
      <button class="ts-customizer-toast-close" aria-label="Toast schließen">×</button>
    `;
    
    // Add click-to-dismiss
    const closeBtn = toast.querySelector('.ts-customizer-toast-close');
    closeBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      toast.remove();
    });
    
    // Also allow clicking anywhere on toast to dismiss
    ...
    console.log('[Tagesspiegel Filter] Comment sort toast shown');
    ...
    }, CORAL_CONFIG.toastDuration);
  }

  function setupCoralMessageListener() {
    browser.runtime.onMessage.addListener((message) => {
      if (message && message.type === 'CORAL_AUTO_SORT_FAILED') {
        showCommentSortToast();
      }
    });

    // Reset toast flag on page navigation (SPA)
    window.addEventListener('popstate', () => {
      CORAL_CONFIG.hasShownToast = false;
    });
  }

  // ========== End Coral Comment Sort Feature ==========
  ...
  async function init() {
    ...
    setupCoralMessageListener(); // Coral comment sort feature
  }
